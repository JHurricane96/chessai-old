MAX_ITER_MTD = 100 # Maximum number of iterations in MTD
MAX_DEPTH = 2 # Maximum depth of negamax
MAX_SCORE = 1000 # Maximum score that is awarded in static evaluation

ALPHA_INIT = 0.002 # Initial learning rate
ALPHA_DEC_FACTOR = 1.003 # Learning rate gets divided by this after every game
LAMBDA = 0.7 # Temporal decay factor
MAX_GAMES = 1000 # Number of games to learn from